...
const formatOrcid = (str: string | undefined) => {
  ...
  return `https://orcid.org/${str.replace(
    new RegExp(`^https://orcid.org/`),
    ""
  )}`;
};
...
export class RoCrateTransformer implements BaseTransformer {
  ...
  private mapComponent(component: ResearchObjectV1Component): RoCrateGraph {
    ...
    let crateComponent: Omit<RoCrateGraph, "@type"> = {
      "@id": component.id,
      name: component.name,
      ...(commonPayload.licenseType
        ? { license: licenseToUrl(commonPayload.licenseType) }
        : {}),
      ...(commonPayload.description
        ? { description: commonPayload.description }
        : {}),
      ...(commonPayload.keywords
        ? { keywords: commonPayload.keywords.join(", ") }
        : {}),
    };
    ...
    if (component.type === ResearchObjectComponentType.PDF) {
      ...
      (creativeWork as any)["/"] = cleanupUrlOrCid(
        (component.payload as any).url
      );
      creativeWork.url = `https://ipfs.io/ipfs/${cleanupUrlOrCid(
        (component.payload as any).url
      )}`;
    ...
    if (component.type === ResearchObjectComponentType.DATA_BUCKET) {
      ...
      dataset["@id"] = "./root";
      dataset["hasPart"] = this.nodeObject!.components.filter(
        (d) => d.type === ResearchObjectComponentType.DATA
      ).map((d) => ({ "@id": d.id }));
      crateComponent = dataset;
    }
