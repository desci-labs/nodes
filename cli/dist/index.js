...
import { getApiKey, getEnvironment, getEnvConfig, initializeNodesLib } from "./config.js";
import { createPushCommand, createPullCommand, createConfigCommand, createListCommand, createPublishCommand, } from "./commands/index.js";
// Initialize nodes-lib with stored configuration
initializeNodesLib();
const program = new Command();
program
    .name("nodes-cli")
    .description(chalk.dim("DeSci Nodes CLI - Push, pull, and publish research data to decentralized nodes"))
    .version("0.1.0")
    .hook("preAction", (thisCommand) => {
    // Show banner for main commands (not for help/version)
    const commandName = thisCommand.args[0];
    if (commandName &&
        !["help", "version", "-h", "--help", "-v", "--version"].includes(commandName)) {
        // Show minimal header
...
program
    ...
    if (apiKey) {
        try {
            ...
            const match = nodes.find((n) => n.uuid === nodeArg || n.uuid.startsWith(nodeArg));
            if (match) {
...
program
    ...
    .action(async (path, node, options) => {
