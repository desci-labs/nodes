...
import { createSpinner, printSuccess, printError, printNodeInfo, symbols, } from "../ui.js";
import { getApiKey, getEnvConfig } from "../config.js";
import { createDraftNode, getDraftNode, listNodes, retrieveDraftFileTree, deleteData, prePublishDraftNode, uploadFiles, createNewFolder, addPdfComponent, } from "@desci-labs/nodes-lib/node";
...
function collectRemotePaths(items, prefix = "") {
    const paths = new Set();
...
async function collectLocalPaths(folderPath) {
...
function findFirstPdf(items, prefix = "") {
    for (const item of items) {
...
export function createPushCommand() {
    return new Command("push")
        .description("Push a folder or files to a DeSci node (overwrites existing files)")
        ...
        .option("--clean", "Remove remote files that don't exist locally (like rsync --delete)")
        ...
        try {
            ...
            console.log(`${symbols.info} ${chalk.bold("Type:")} ${isDirectory ? "Folder" : "File"}`);
            if (options.dryRun) {
                console.log(chalk.yellow(`${symbols.warning} Dry run mode - no changes will be made`));
            }
            console.log();
            ...
            try {
                ...
                spinner.succeed(`Target: ${chalk.cyan(node.title)} ${isNewNode ? chalk.green("(new)") : chalk.yellow("(updating)")}`);
            }
            catch {
            ...
            if (isDirectory && !isNewNode) {
                ...
                const updatedFiles = [...localPaths].filter((p) => remotePaths.has(p));
                console.log();
                console.log(chalk.bold("Changes to be applied:"));
                console.log(`  ${chalk.green("+")} ${newFiles.length} new files`);
                console.log(`  ${chalk.yellow("~")} ${updatedFiles.length} files to update/overwrite`);
                if (options.clean) {
                    console.log(`  ${chalk.red("-")} ${filesToDelete.length} files to delete`);
                }
                if (options.dryRun) {
                    console.log();
                    ...
                    if (updatedFiles.length > 0) {
                        ...
                        if (updatedFiles.length > 10)
                            console.log(chalk.dim(`  ... and ${updatedFiles.length - 10} more`));
                    }
                    if (filesToDelete.length > 0) {
                        ...
                        if (filesToDelete.length > 10)
                            console.log(chalk.dim(`  ... and ${filesToDelete.length - 10} more`));
                    }
            ...
            if (filesToDelete.length > 0 && !options.dryRun) {
                const deleteSpinner = createSpinner(`Removing ${filesToDelete.length} old files...`);
                deleteSpinner.start();
                let deleted = 0;
            ...
            try {
                ...
                uploadSpinner.succeed(`Upload complete in ${Math.round(duration / 1000)}s`);
                ...
                if (options.prepublish) {
                    ...
                    try {
                        ...
                        console.log(`${symbols.info} Updated CID: ${chalk.dim(prepubResult.updatedManifestCid)}`);
                    }
                    catch {
                ...
                if (options.prepublish) {
                    printSuccess(isNewNode
                        ? "Node created and new version prepared!"
                        : "Node updated to new version!");
                }
                else {
                    printSuccess(isNewNode ? "Node created and files uploaded!" : "Draft updated!");
                    console.log(chalk.yellow(`\n${symbols.warning} Changes are in draft state.`));
                    console.log(chalk.dim("   To finalize as a new version, use --prepublish flag or run publish command"));
                }
                const { webUrl } = getEnvConfig();
                console.log(chalk.dim("\nNext steps:"));
                console.log(chalk.dim(`  • View in browser:  ${webUrl}/node/${targetUuid}`));
                console.log(chalk.dim(`  • Pull files:       nodes-cli pull ${targetUuid.slice(0, 8)}`));
                console.log(chalk.dim(`  • Publish node:     nodes-cli publish ${targetUuid.slice(0, 8)}`));
            }
            catch (err) {
