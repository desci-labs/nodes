...
import { createSpinner, printError, printTable, truncateUuid, formatBytes, symbols, } from "../ui.js";
import { getApiKey, getEnvConfig } from "../config.js";
import { listNodes, retrieveDraftFileTree, } from "@desci-labs/nodes-lib/node";
...
async function listAllNodes(options) {
...
async function listNodeFiles(nodeArg, options) {
    // Find matching node
    const spinner = createSpinner("Finding node...");
    spinner.start();
    const { nodes } = await listNodes();
    const matches = nodes.filter((n) => n.uuid === nodeArg ||
    ...
    try {
        ...
        if (tree.length === 0) {
            ...
            console.log(chalk.dim("Add files with: nodes-cli push <folder> --node " +
                node.uuid.slice(0, 8) +
                "\n"));
            return;
        }
