...
import { createSpinner, printSuccess, printError, printNodeInfo, formatBytes, symbols, } from "../ui.js";
import { getApiKey, getEnvConfig } from "../config.js";
import { getDraftNode, listNodes, retrieveDraftFileTree, } from "@desci-labs/nodes-lib/node";
...
async function downloadTree(item, basePath, onProgress, progress) {
    if (item.type === "file") {
...
export function createPullCommand() {
    return new Command("pull")
        ...
        try {
            ...
            const itemsToDownload = targetItem.type === "dir" && targetItem.contains
                ? targetItem.contains
                : [targetItem];
            ...
            try {
                for (const item of itemsToDownload) {
                    await downloadTree(item, outputPath, (p) => {
                        downloadSpinner.text = `Downloading ${chalk.cyan(p.currentFile)} (${p.current}/${p.total}) - ${formatBytes(p.downloadedBytes)}`;
                    }, progress);
                }
                const duration = Date.now() - startTime;
                downloadSpinner.succeed(`Downloaded ${progress.current} files (${formatBytes(progress.downloadedBytes)}) in ${Math.round(duration / 1000)}s`);
