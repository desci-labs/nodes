...
`;
    ...
    console.log(chalk.dim("  Push, pull & publish research data to decentralized nodes\n"));
}
...
export function printFileList(files, type) {
...
export function printTable(headers, rows, options) {
    ...
    const colWidths = headers.map((h, i) => Math.min(Math.max(h.length, ...rows.map((r) => (r[i] || "").length)), Math.floor(maxWidth / headers.length)));
...
export function truncateUuid(uuid, length = 8) {
    if (uuid.length <= length)
        return uuid;
    return uuid.slice(0, length) + "...";
}
export function maskString(str, visibleEnd = 4) {
    if (str.length <= visibleEnd)
