...
echo "# this docker-compose.redis.yml file is overwritten on startup, make changes to genereateRedisCompose.sh instead" >> docker-compose.redis.yml
...
if [ "$REDIS_MODE" = "cluster" ]; then
  ...
  echo "  command: sh -c 'mkdir -p /redis && redis-server /redis/redis.conf --port \$\$PORT --requirepass \$\$REDIS_PASSWORD --masterauth \$\$REDIS_PASSWORD'" >> docker-compose.redis.yml
...
for ((i=0; i<$redis_nodes; i++)); do
  for ((j=0; j<$redis_replicas; j++)); do
    ...
    service_name="redis-replica-$((i * redis_replicas + j + 1))"
    redis_services+="  $service_name:
    <<: *redis-node
    ports:
      - '$port:$port'
...
"
  ...
  redis_services+="  redis-cluster-creator:
      ...
      command: redis-cli -a $REDIS_PASSWORD -p $start_port --cluster create $cluster_command --cluster-replicas $redis_replicas --cluster-yes
      depends_on:
$depends_on
"
