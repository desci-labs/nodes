...
export const signerFromPkey = (pkey: string): Signer => walletFromPkey(pkey);
...
const getManualSignatureAuthMethod =
  (accountId: AccountId, signer: Signer): AuthMethod =>
  async (opts: AuthMethodOpts) => {
    const now = new Date();
...
export const authorizedSessionDidFromSigner = async (
  ...
  const externalProvider = (signer.provider as providers.Web3Provider)
    ?.provider;
