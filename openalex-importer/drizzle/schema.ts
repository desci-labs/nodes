import { pgTable, pgSchema, text, real, bigint, json, integer, timestamp, index, boolean, date, vector } from "drizzle-orm/pg-core"
...
(table) => {
	return {
		referenced_work_id_idx: index("works_referenced_works_referenced_work_id_idx").using("btree", table.referenced_work_id),
...
(table) => {
	return {
		descriptions_embeddings_idx: index("concepts_descriptions_embeddings_idx").using("ivfflat", table.descriptions_embeddings.op("vector_l2_ops")).with({lists: "100"}),
		name_embeddings_idx: index("concepts_name_embeddings_idx").using("ivfflat", table.name_embeddings.op("vector_l2_ops")).with({lists: "100"}),
	}
});
...
(table) => {
	return {
		...
		related_concept_id_idx: index("concepts_related_concepts_related_concept_id_idx").using("btree", table.related_concept_id),
	}
});