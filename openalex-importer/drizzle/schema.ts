...
export const publishers_counts_by_yearInOpenalex = openalex.table(
...
export const institutions_associated_institutionsInOpenalex = openalex.table(
...
export const works_primary_locationsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      work_id_idx: index("works_primary_locations_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const worksInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      publication_date_idx: index("works_publication_date_idx").using(
        "btree",
...
export const institutions_counts_by_yearInOpenalex = openalex.table(
...
export const sources_counts_by_yearInOpenalex = openalex.table(
...
export const works_conceptsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      concept_id_idx: index("works_concepts_concept_id_idx").using(
        "btree",
        table.concept_id
      ),
      work_id_idx: index("works_concepts_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_meshInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      descriptor_ui_idx: index("works_mesh_descriptor_ui_idx").using(
        "btree",
        table.descriptor_ui
      ),
      qualifier_ui_idx: index("works_mesh_qualifier_ui_idx").using(
        "btree",
        table.qualifier_ui
      ),
      work_id_idx: index("works_mesh_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_open_accessInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      work_id_idx: index("works_open_access_work_id_idx").using(
        "btree",
        table.work_id
      ),
      work_id_idx1: index("works_open_access_work_id_idx1").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_locationsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      work_id_idx: index("works_locations_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_referenced_worksInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      referenced_work_id_idx: index(
        "works_referenced_works_referenced_work_id_idx"
      ).using("btree", table.referenced_work_id),
      work_id_idx: index("works_referenced_works_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_related_worksInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      related_work_id_idx: index(
        "works_related_works_related_work_id_idx"
      ).using("btree", table.related_work_id),
      work_id_idx: index("works_related_works_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_topicsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      topic_id_idx: index("works_topics_topic_id_idx").using(
        "btree",
        table.topic_id
      ),
      work_id_idx: index("works_topics_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const authors_counts_by_yearInOpenalex = openalex.table(
...
export const conceptsInOpenalex = openalex.table(
  "concepts",
  {
    ...
    works_api_url: text("works_api_url"),
    updated_date: timestamp("updated_date", { mode: "string" }),
    descriptions_embeddings: vector("descriptions_embeddings", {
      dimensions: 768,
    }),
...
export const concepts_ancestorsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      concept_id_idx: index("concepts_ancestors_concept_id_idx").using(
        "btree",
        table.concept_id
      ),
    };
...
export const concepts_counts_by_yearInOpenalex = openalex.table(
...
export const works_authorshipsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      author_id_idx: index("works_authorships_author_id_idx").using(
        "btree",
        table.author_id
      ),
      institution_id_idx: index("works_authorships_institution_id_idx").using(
        "btree",
        table.institution_id
      ),
      work_id_idx: index("works_authorships_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const works_best_oa_locationsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      work_id_idx: index("works_best_oa_locations_work_id_idx").using(
        "btree",
        table.work_id
      ),
    };
...
export const concepts_related_conceptsInOpenalex = openalex.table(
  ...
  (table) => {
    return {
      concept_id_idx: index("concepts_related_concepts_concept_id_idx").using(
        "btree",
        table.concept_id
      ),
      related_concept_id_idx: index(
        "concepts_related_concepts_related_concept_id_idx"
      ).using("btree", table.related_concept_id),
    };
  }
);

export const batchesInOpenAlex = openalex.table("batch", {
  id: serial("id").primaryKey(),
  createdAt: timestamp("createdAt", { mode: "date" }),
  updatedAt: timestamp("updatedAt", { mode: "date" }),
});

export const workBatchesInOpenAlex = openalex.table("works_batch", {
  work_id: text("work_id")
