...
if [[ "${ENVIRONMENT}" = 'test' ]]; then
    ...
    yarn compile
    sed -i '' "s/<DATABASE_URL>/postgresql:\/\/walter:white@host.docker.internal:5434\/boilerplate/g" '.wrangler/dist/index.js'
    yarn build
else
    awk '{gsub("<DATABASE_URL>", "postgresql://walter:white@host.docker.internal:5433/boilerplate", $0); print}' template.toml >wrangler.toml
    yarn compile
    sed -i '' "s/<DATABASE_URL>/postgresql:\/\/walter:white@host.docker.internal:5433\/boilerplate/g" '.wrangler/dist/index.js'
    yarn build
fi
