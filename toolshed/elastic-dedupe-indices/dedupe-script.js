...
async function refreshIndex() {
  try {
    if (config.waitForRefresh) {
      ...
      client.indices.refresh({ index: config.openIndex })
...
async function fetchOpenIdsWithScroll(scrollId = null, batchId, retryCount = 0) {
  try {
    let response;
    ...
    if (error.message.includes('search_context_missing_exception') || 
      ...
      log(`Lost scroll context, retrying with new search (retry ${retryCount + 1})`);
...
async function checkIdsInClosedIndex(ids, batchId) {
  ...
  if (config.parallelMget) {
    ...
    while (nextBatchIndex < subBatches.length || runningBatches.size > 0) {
      ...
      while (runningBatches.size < config.mgetMaxParallel && nextBatchIndex < subBatches.length) {
        ...
        (async () => {
          try {
            log(`Batch ${batchId}: Checking sub-batch ${batchIndex+1}/${subBatches.length} (${batchIds.length} IDs)`);
            ...
            if (existingIds.length > 0) {
              ...
              log(`Batch ${batchId}, sub-batch ${batchIndex+1}: Found ${existingIds.length} duplicates`);
            }
          } catch (error) {
            log(`Error checking sub-batch ${batchIndex+1} of ${batchId}: ${error.message}`, 'ERROR');
          } finally {
    ...
    for (let i = 0; i < subBatches.length; i++) {
      ...
      log(`Batch ${batchId}: Checking ${batchIds.length} IDs in closed index (${i+1}-${i+batchIds.length}/${ids.length})`);
      ...
      try {
        ...
        log(`Batch ${batchId}: Found ${existingIds.length} duplicates in this sub-batch`);
      } catch (error) {
  ...
  log(`Batch ${batchId}: Found total of ${duplicates.length} duplicates in closed index`);
  return duplicates;
}
...
async function deleteDuplicatesFromOpen(duplicateIds, batchId) {
  ...
  try {
    log(`Batch ${batchId}: Scheduling deletion of ${duplicateIds.length} duplicates from open index`);
    ...
    if (!config.waitForCompletion) {
      ...
      log(`Batch ${batchId}: Deletion scheduled as task ${taskId} (not waiting for completion)`);
      ...
      const deleted = bulkResponse.items.filter(item => 
        item.delete?.result === 'deleted' || 
        item.delete?.status === 200 || 
        item.delete?.status === 201
      ).length;
      
      log(`Batch ${batchId}: Successfully deleted ${deleted}/${duplicateIds.length} duplicates`);
    ...
    try {
      ...
      log(`Failed to delete even with smaller batches: ${subError.message}`, 'ERROR');
...
async function deleteInSmallBatches(ids, batchId) {
  ...
  log(`Splitting batch ${batchId} into ${chunks.length} chunks of ~${chunkSize} IDs each`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    try {
      ...
      log(`Chunk ${i+1}/${chunks.length}: Deleted ${deleted}/${chunks[i].length} documents`);
    } catch (error) {
...
const taskTracker = {
  ...
  cleanup() {
    const now = Date.now();
    for (const [id, task] of this.tasks.entries()) {
      if (task.completed && task.completionTime && (now - task.completionTime > 3600000)) {
...
async function checkTaskStatus() {
      ...
      try {
        ...
        if (taskInfo.completed) {
          ...
          if (task && (Date.now() - task.startTime > config.taskTimeout)) {
            log(`Task ${taskId} timed out after ${config.taskTimeout/1000}s, marking as failed`, 'WARN');
  ...
  log(`Task status: ${stats.completed} completed, ${stats.inProgress} in progress, ${stats.failed} failed, ${stats.totalDeleted} docs deleted`);
...
async function dedupOpenIndex() {
  ...
  try {
    ...
    if (checkpoint && checkpoint.scrollId) {
      ...
      log(`Resuming from checkpoint, already processed ${totalProcessed} IDs, found ${totalDuplicatesFound} duplicates, deleted ${totalDeleted}`);
    }
    ...
    const statusTimer = setInterval(() => {
      ...
      log(`STATUS: Processed ${totalProcessed} IDs (${rate}/min), found ${totalDuplicatesFound} duplicates, deleted ${totalDeleted}. Active batches: ${inProgress.size}, Queue: ${batchQueue.length}`);
    ...
    while (hasMore || batchQueue.length > 0 || inProgress.size > 0) {
      ...
      if (hasMore && batchQueue.length < config.fetchAhead) {
        ...
        log(`Fetching batch ${batchId} from open index${scrollId ? ' (with scroll)' : ''}`);
        ...
        if (result.error) {
          ...
          const backoffMs = Math.min(
            config.maxBackoffMs, 
            config.backoffTimeMs * Math.pow(config.backoffFactor, Math.min(5, batchCount % 10))
          );
      ...
      while (batchQueue.length > 0 && inProgress.size < config.concurrentBatches) {
        ...
        (async () => {
          try {
            ...
            log(`Processing batch ${batch.batchId}: Checking ${batch.ids.length} IDs`);
            const duplicateIds = await checkIdsInClosedIndex(batch.ids, batch.batchId);
            
            // Step 2: Delete duplicates from open index
            if (duplicateIds.length > 0) {
              log(`Batch ${batch.batchId}: Found ${duplicateIds.length} duplicates, deleting from open index`);
              const deleteResult = await deleteDuplicatesFromOpen(duplicateIds, batch.batchId);
              ...
              if (deleteResult.success) {
                if (config.waitForCompletion) {
                  ...
                  log(`Batch ${batch.batchId}: Scheduled deletion of ${duplicateIds.length} duplicates (task: ${deleteResult.taskId})`);
                }
              }
            } else {
            ...
            const minutesSinceCheckpoint = (Date.now() - lastCheckpointTime) / 60000;
            
            if (batchCount % config.checkpointInterval === 0 || minutesSinceCheckpoint >= config.checkPointEveryNMinutes) {
              saveCheckpoint({
                scrollId,
                totalProcessed,
              ...
              const processedSinceStart = totalProcessed - startProcessed;
              const rate = (processedSinceStart / parseFloat(elapsedMinutes)).toFixed(2);
              log(`Progress: Processed ${totalProcessed} IDs (${rate}/min), found ${totalDuplicatesFound} duplicates, deleted ${totalDeleted}`, 'INFO', true);
            ...
            log(`Error processing batch ${batch.batchId}: ${error.message}`, 'ERROR');
          } finally {
    ...
    if (!config.waitForCompletion) {
      ...
      while (taskTracker.hasActiveTasks()) {
        await checkTaskStatus();
        await new Promise(resolve => setTimeout(resolve, config.tasksCheckInterval));
      }
      ...
      log(`All deletion tasks completed, total deleted from tasks: ${stats.totalDeleted}`);
    }
    ...
    const finalRate = ((totalProcessed - startProcessed) / (duration / 60)).toFixed(2);
    log(`Deduplication complete. Processed ${totalProcessed} IDs (${finalRate}/min), found ${totalDuplicatesFound} duplicates, deleted ${totalDeleted} in ${Math.ceil(duration/60)} minutes`);
...
async function main() {
  ...
  try {
    ...
    if (!idFieldDataEnabled) {
      log("WARNING: Could not enable id_field_data. This is required for the script to work.", 'ERROR');
      process.exit(1);
    }
    ...
    if (result.success) {
      ...
      log(`Summary: Processed ${result.totalProcessed} IDs (${result.rate}/min), found ${result.totalDuplicatesFound} duplicates, deleted ${result.totalDeleted} in ${(result.duration / 60).toFixed(2)} minutes`);
      process.exit(0);
    } else {
