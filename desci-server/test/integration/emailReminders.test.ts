...
describe('Email Reminder Handlers', () => {
  ...
  describe('checkProChatRefresh', () => {
    it('should identify PRO users with expired period (>30 days)', async () => {
      // Setup: PRO plan with expired period
      await prisma.userFeatureLimit.create({
        data: {
          userId: testUser.id,
          ...
          period: Period.MONTH,
          currentPeriodStart: subDays(new Date(), 35), // 35 days ago (expired)
        },
      });

      // Run handler
      const result = await checkProChatRefresh.handler();
    ...
    it('should identify PRO users with just-refreshed period (0-1 days)', async () => {
      // Setup: PRO plan with just-refreshed period
      await prisma.userFeatureLimit.create({
        data: {
          userId: testUser.id,
          planCodename: PlanCodename.PRO,
          ...
          useLimit: null, // Unlimited
          period: Period.MONTH,
          currentPeriodStart: subHours(new Date(), 12), // 12 hours ago (just refreshed)
        },
      });
    ...
    it('should skip PRO users with mid-period state (2-29 days)', async () => {
      // Setup: PRO plan mid-period
      await prisma.userFeatureLimit.create({
        data: {
          userId: testUser.id,
          planCodename: PlanCodename.PRO,
          ...
          useLimit: null,
          period: Period.MONTH,
          currentPeriodStart: subDays(new Date(), 15), // 15 days ago (mid-period)
        },
      });
    ...
    it('should skip PRO users who received email in last 30 days', async () => {
      // Setup: PRO plan with expired period
      await prisma.userFeatureLimit.create({
        data: {
          userId: testUser.id,
          ...
          useLimit: null,
          period: Period.MONTH,
          currentPeriodStart: subDays(new Date(), 35),
        },
      });

      // Setup: Already sent email 10 days ago
      await prisma.sentEmail.create({
        data: {
          userId: testUser.id,
