...
export async function initializeTrialForNewUser(userId: number): Promise<void> {
  ...
  // Create usage record with initial credits
  await prisma.usage.create({
    data: {
      userId,
      subscriptionId: subscription.id,
      feature: Feature.RESEARCH_ASSISTANT,
      totalLimit: INITIAL_TRIAL_CREDITS,
      totalUsed: 0,
      totalRemaining: INITIAL_TRIAL_CREDITS,
      planCodename: PlanCodename.FREE,
      initialTrialCredits: INITIAL_TRIAL_CREDITS,
    },
  });

  logger.info(`Initialized trial for user ${userId}: ${trialStart.toISOString()} to ${trialEnd.toISOString()}`);
}
...
async function refreshDailyCreditIfNeeded(
  userId: number,
  usage: { id: number; totalUsed: number; totalRemaining: number | null; initialTrialCredits: number | null; nextCreditRefreshTime: Date | null },
...
function formatUsageData(
  ...
  return {
    ...
    planCodename: (usage.planCodename as PlanCodename) || (subscription.planType === PlanType.PREMIUM ? PlanCodename.PREMIUM : PlanCodename.FREE),
