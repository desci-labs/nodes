...
export async function hardDeleteUser(userId: number): Promise<void> {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (authoredSubmissionIds.length > 0) {
      const assignmentIds = (
        await tx.refereeAssignment.findMany({
          where: { submissionId: { in: authoredSubmissionIds } },
          select: { id: true },
        })
      ).map((a) => a.id);
      await tx.journalFormResponse.deleteMany({
        where: {
          OR: [
            { journalSubmissionId: { in: authoredSubmissionIds } },
            ...(assignmentIds.length > 0 ? [{ refereeAssignmentId: { in: assignmentIds } }] : []),
          ],
        },
      });
      await tx.refereeInvite.deleteMany({ where: { submissionId: { in: authoredSubmissionIds } } });
      await tx.journalSubmissionReview.deleteMany({ where: { submissionId: { in: authoredSubmissionIds } } });
      await tx.refereeAssignment.deleteMany({ where: { submissionId: { in: authoredSubmissionIds } } });
      await tx.journalEventLog.deleteMany({ where: { submissionId: { in: authoredSubmissionIds } } });
      await tx.journalSubmissionRevision.deleteMany({ where: { submissionId: { in: authoredSubmissionIds } } });
      await tx.journalSubmission.deleteMany({ where: { authorId: userId } });
    }
    await tx.journalSubmission.updateMany({ where: { assignedEditorId: userId }, data: { assignedEditorId: null } });
    ...
    // 9. InteractionLog (references User)
    await tx.interactionLog.deleteMany({ where: { userId } });
    ...
    if (nodeIds.length > 0) {
      ...
      if (nodeDpids.length > 0) {
        ...
        if (nodeLinkedSubmissionIds.length > 0) {
          const nodeAssignmentIds = (
            await tx.refereeAssignment.findMany({
              where: { submissionId: { in: nodeLinkedSubmissionIds } },
              select: { id: true },
            })
          ).map((a) => a.id);
          await tx.journalFormResponse.deleteMany({
            where: {
              OR: [
                { journalSubmissionId: { in: nodeLinkedSubmissionIds } },
                ...(nodeAssignmentIds.length > 0 ? [{ refereeAssignmentId: { in: nodeAssignmentIds } }] : []),
              ],
            },
          });
          await tx.refereeInvite.deleteMany({
            where: { submissionId: { in: nodeLinkedSubmissionIds } },
          });
          await tx.journalSubmissionReview.deleteMany({
            where: { submissionId: { in: nodeLinkedSubmissionIds } },
          });
          await tx.refereeAssignment.deleteMany({
            where: { submissionId: { in: nodeLinkedSubmissionIds } },
          });
          await tx.journalEventLog.deleteMany({
            where: { submissionId: { in: nodeLinkedSubmissionIds } },
          });
          await tx.journalSubmissionRevision.deleteMany({
            where: { submissionId: { in: nodeLinkedSubmissionIds } },
          });
        }
    ...
    logger.info({ userId, durationMs: new Date().getTime() - start.getTime() }, 'Hard delete completed');
  });
