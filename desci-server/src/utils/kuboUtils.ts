...
import { code as MULTIFORMATS_RAW_CODE } from 'multiformats/codecs/raw';

// // ===========================================================================
export async function getSize(ipfs: IPFSHTTPClient, cid: string | CID, allowDir = false) {
  if (typeof cid === 'string') cid = CID.parse(cid);
  const block = await ipfs.block.get(cid);
  // console.log('cid: ', cid);
  // console.log('cid.code: ', cid.code);
  // if raw, use length of block
  if (cid.code == MULTIFORMATS_RAW_CODE) {
    return block.length;
  }
...
export async function addToDir(ipfs, dirCid, files) {
  if (dirCid.code == MULTIFORMATS_RAW_CODE) {
...
export async function updateDagCid(ipfs, nodeCid, oldCid, newCid) {
  ...
  if (nodeCid.code == MULTIFORMATS_RAW_CODE) {
    throw new Error('raw cid -- not a directory');
  }
